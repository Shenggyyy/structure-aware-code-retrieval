...
const state = { token: null, repositories: [], history: [], busy: true, job: null, run: null, poll: null };
const strategies = [
  ["bm25", "BM25", "关键词与标识符匹配"],
  ["dense", "Dense", "向量语义检索"],
...
function stage(name, status) {
  const good = ["ready", "reused", "completed", "preview", "preview_complete"].includes(status);
  const bad = ["failed", "interrupted", "unreadable"].includes(status);
...
function renderJob(job) {
  ...
  } else if (progress.phase === "preview") {
...
async function pollJob(id) {
  ...
  try {
    ...
    const runId = job.result?.run_id || (job.kind === "preview" ? job.progress?.detail?.run_id : null);
...
function renderCard(key, name, description, row) {
  ...
  metric(grid, "检索耗时", ms(row.timing_ms?.retrieval));
  ...
  metric(grid, "模型 API 调用", text(row.cost?.model_calls));
  body.append(grid);
  ...
  details.append(node("summary", "", "更多运行信息"), node("pre", "", JSON.stringify({ timing_ms: row.timing_ms, tokens: row.tokens, cost: row.cost, ranked_candidates: row.ranked_candidates, structure_stats: row.structure_stats, automatic_checks: checks ?? null }, null, 2)));
  body.append(details);
  return card;
}
function renderRun(run, current) {
  ...
  $("run-mode").textContent = current ? "本轮上下文预览 · 已自动保存" : "历史上下文预览 · 已保存结果";
  ...
  $("run-config").textContent = JSON.stringify({ mode: run.mode, settings: run.settings, source: run.resources?.source, snapshot_id: run.resources?.snapshot_id, resource_id: run.resources?.resource_id, index_config: run.resources?.index_config, encoder_mode: run.encoder_mode, model_spec: run.resources?.model_spec, resolver_config: run.resources?.resolver_config, shared_setup: run.shared_setup, relevance_labels: run.relevance_labels, benchmark_metrics: run.benchmark_metrics }, null, 2);
}
...
function renderHistory() {
  ...
  for (const row of records) {
    ...
    description.append(node("h3", "", row.question || "无法读取的记录"), node("p", "", `${date(row.created_at)} · ${statusText(row.status)} · 已保存上下文预览`), node("p", "mono", `run ${row.run_id} · ${repository?.source?.location || `repository ${short(row.repository_id)}`}`));
