...
当前约定范围已正式交付：核心系统、实验流程、M9 浏览器工作流、中英文首页和双语界面。
最终审查及本地验证已完成，研究局限仍明确保留。后续功能或实验属于需要另行提出的可选扩展。
具体验证与剩余限制见[项目状态](docs/status.md)和[交付记录](docs/delivery.md)。
...
较窄的屏幕可以横向滚动查看五列结果。这张第一阶段截图保留了当时的中文界面；
当前页面可通过 **Interface language / 界面语言** 在 English 和简体中文之间切换。
...
2. 点击 **Import and prepare / 导入并准备**，查看源码版本以及解析、向量和关系图的准备状态。项目只静态读取被导入仓库，
   不执行其依赖安装、安装脚本或测试。
3. 输入问题，点击 **Preview all five strategies / 运行五策略预览**，比较检索到的代码证据。某个策略失败或缺失时会如实显示，不补造结果。
4. 如需模型回答，先生成费用计划，核对模型、请求上限和合计估算，再明确确认付费生成与预算。
   启动服务前，仅在服务端本机环境变量中配置 `OPENAI_API_KEY`；修改环境变量后需重启服务。
   不要在网页中输入密钥。
5. 点击回答中的引用查看源码，通过 **History / 历史记录** 重新打开保存的对比。重新打开或刷新页面不会重复付费请求。
...
打开本地工作台，在 **History / 历史记录** 中选择 **How are request URLs prepared?**。
...
浏览器本地化测试使用 `PATH` 中的 Node.js，无需 npm 依赖。通过
`uv run --locked pytest tests/workbench/test_i18n.py` 运行这些契约检查；本地缺少 Node 时，pytest 会跳过。
CI 在测试前明确检查 Node 是否可用。启动工作台服务本身不需要 Node。
